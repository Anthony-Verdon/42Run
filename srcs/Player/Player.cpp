...
#define PLAYER_CAPSULE_HEIGHT_ROLLING 0
...
void Player::OnContactAdded([[maybe_unused]] const JPH::ContactManifold &inManifold, [[maybe_unused]] const PhysicBody3D *collisionedBody)
{
    ...
    else if (collisionedBody->GetPhysicBodyType() == PhysicBodyType::TILE)
    {
        ...
        // -0.1 - 0.1: wall
        // < -0.7: ceiling
        if (ml::dotProduct(ml::vec3(inManifold.mWorldSpaceNormal.GetX(), inManifold.mWorldSpaceNormal.GetY(), inManifold.mWorldSpaceNormal.GetZ()), ml::vec3(0, 1, 0)) <= 0.1)
            playerLost = true;
        else
            onGround = true;
    }
