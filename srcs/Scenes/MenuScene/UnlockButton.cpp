...
UnlockButton::UnlockButton(const std::string &text, const std::string &font, const ml::vec2 &pos, const ml::vec2 &size, const std::function<void(const UI::CallbackData &)> &callback) : Button(text, font, pos, size, callback), background({"rectangle_button_depth_flat", ml::vec2(1, 1), ml::vec2(0, 0), size}), star({"star", ml::vec2(1, 1), ml::vec2(0, 0), ml::vec2(size.y / 2, size.y / 2)})
{
}
